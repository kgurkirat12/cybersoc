...
def logs():
    ...
    threat_only = request.args.get(
        'threat_only',
        '0'
    ) == '1'

    logs_list = get_recent_logs(
        limit=100,
...
def analyze_log_endpoint():
    """Analyzes user submitted or preset security logs."""
    ...
    if not raw_log:
        ...
        if request.is_json:
            ...
            return jsonify({
                'error': 'Raw log string is required'
            }), 400

        flash(
            'Please enter a valid log message to analyze.',
            'danger'
        )
    ...
    if result['is_threat']:
        ...
        flash(
            ...
            f"Threat Detected: "
            f"{result['analysis']['alert_type']} "
            f"({result['analysis']['severity']} Severity)",
...
@app.route('/alerts')
def alerts():
    """Renders Security Alerts page with filters."""
...
@app.route('/ip-intelligence/<path:source_ip>')
def ip_intelligence(source_ip):
    """
    Displays IP Threat Intelligence information
    based on CyberSOC alerts and logs.
    """

    intelligence = get_ip_threat_intelligence(
        source_ip
    )
...
def download_report():
    """Generates and downloads executive PDF security report."""
    ...
    filename = (
        ...
        f"CyberSOC_Security_Report_"

        f"{os.urandom(3).hex()}.pdf"
...
def api_chart_data():
    """REST API endpoint for Chart.js dashboard widgets."""
...
def api_recent_alerts():
    """REST API endpoint for latest security alerts."""
